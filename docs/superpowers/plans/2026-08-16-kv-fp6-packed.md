...
- **Packing (locked):** LSB-first bitstream per token. Code `i` (dim) at bit `[6i, 6i+6)`; byte offset `(3i)>>2`, bit `(6i)&7`. Token code block = `256*6/8 = 192` bytes. A d-block of 8 consecutive dims (`d%8==0`) is 48 bits at byte offset `(d/8)*6`: load `uint64` (8 bytes, unaligned OK), code j = `(raw >> (6*j)) & 0x3F`. Encode 8 codes → 6 bytes at `(d/8)*6` (build uint64, store bytes `[6k,6k+4)` as uint + bytes `[6k+4,6k+6)` as ushort). A 64-group = 48 bytes.
